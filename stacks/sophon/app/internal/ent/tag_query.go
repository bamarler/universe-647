...
func (_q *TagQuery) QueryTasks() *TaskQuery {
...
func (_q *TagQuery) QueryNotes() *NoteQuery {
...
func (_q *TagQuery) QueryProjectTasks() *TaskQuery {
	query := (&TaskClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *TagQuery) WithTasks(opts ...func(*TaskQuery)) *TagQuery {
	query := (&TaskClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TagQuery) WithNotes(opts ...func(*NoteQuery)) *TagQuery {
	query := (&NoteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TagQuery) WithProjectTasks(opts ...func(*TaskQuery)) *TagQuery {
	query := (&TaskClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := _q.withTasks; query != nil {
		if err := _q.loadTasks(ctx, query, nodes,
			func(n *Tag) { n.Edges.Tasks = []*Task{} },
			func(n *Tag, e *Task) { n.Edges.Tasks = append(n.Edges.Tasks, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withNotes; query != nil {
		if err := _q.loadNotes(ctx, query, nodes,
			func(n *Tag) { n.Edges.Notes = []*Note{} },
			func(n *Tag, e *Note) { n.Edges.Notes = append(n.Edges.Notes, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withProjectTasks; query != nil {
		if err := _q.loadProjectTasks(ctx, query, nodes,
			func(n *Tag) { n.Edges.ProjectTasks = []*Task{} },
			func(n *Tag, e *Task) { n.Edges.ProjectTasks = append(n.Edges.ProjectTasks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TagQuery) loadTasks(ctx context.Context, query *TaskQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Task)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.TasksTable)
		s.Join(joinT).On(s.C(task.FieldID), joinT.C(tag.TasksPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.TasksPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.TasksPrimaryKey[1]))
...
func (_q *TagQuery) loadNotes(ctx context.Context, query *NoteQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Note)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Tag)
	nids := make(map[int]map[*Tag]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.NotesTable)
		s.Join(joinT).On(s.C(note.FieldID), joinT.C(tag.NotesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.NotesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.NotesPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (_q *TagQuery) loadProjectTasks(ctx context.Context, query *TaskQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Task)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Tag)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Task(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tag.ProjectTasksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.task_project
		if fk == nil {
			return fmt.Errorf(`foreign-key "task_project" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "task_project" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
