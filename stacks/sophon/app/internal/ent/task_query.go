...
func (_q *TaskQuery) QueryProject() *TagQuery {
...
func (_q *TaskQuery) WithProject(opts ...func(*TagQuery)) *TaskQuery {
	query := (&TagClient{config: _q.config}).Query()
...
func (_q *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := _q.withProject; query != nil {
		if err := _q.loadProject(ctx, query, nodes, nil,
			func(n *Task, e *Tag) { n.Edges.Project = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *TaskQuery) loadProject(ctx context.Context, query *TagQuery, nodes []*Task, init func(*Task), assign func(*Task, *Tag)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Task)
	for i := range nodes {
		if nodes[i].task_project == nil {
			continue
		}
		fk := *nodes[i].task_project
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "task_project" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TaskQuery) loadTags(ctx context.Context, query *TagQuery, nodes []*Task, init func(*Task), assign func(*Task, *Tag)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(task.TagsTable)
		s.Join(joinT).On(s.C(tag.FieldID), joinT.C(task.TagsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(task.TagsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(task.TagsPrimaryKey[0]))
